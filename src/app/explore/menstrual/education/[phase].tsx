...
import { ChevronLeft, Droplet, Leaf, Sun, Moon, Calendar } from "lucide-react-native";
import { useLocalSearchParams, router } from "expo-router";
import DarkScreen from "../../../../components/menstrual/DarkScreen";
import PhaseTabs, { PhaseTab } from "../../../../components/menstrual/PhaseTabs";
...
const phaseContent = {
  menstrual: {
    ...
    overview: {
      whatHappens: "Menstrual bleeding occurs, and the uterine lining is shed. You may experience cramps, fatigue, headache, bloating and body discomfort. Experiences vary between women.",
    ...
    food: {
      keyNutrient: "Iron",
      description: "Iron helps replenish what is lost during menstrual bleeding.",
      vegetarian: ["Leafy greens (spinach, methi, etc.)", "Dal, chana, rajma", "Pulses", "Sesame seeds", "Nuts and dry fruits"],
  ...
  follicular: { 
    ...
    bg: "bg-[#A7E3A1]/20", 
    overview: { 
      whatHappens: "This phase follows the beginning of menstruation and continues towards ovulation. Ovarian follicles develop and hormonal changes occur. Some women may notice increasing energy as the phase progresses.", 
      duration: "This phase occurs after menstruation and continues until ovulation. The duration varies.", 
      image: require("../../../../assets/images/explore/menstrual_phase_illustration.jpg") 
    }, 
    food: { 
      keyNutrient: "Protein & Whole Grains", 
      description: "Focus on a balanced diet containing Protein, Whole grains, Millets, Pulses, Vegetables, Fruits, Nuts, Seeds, Healthy fats, and adequate fluids.", 
      vegetarian: [
        "Dal & Chana",
        "Rajma",
        "Paneer & Curd",
  ...
  ovulation: { 
    ...
    bg: "bg-[#F9E076]/20", 
    overview: { 
      whatHappens: "Ovulation is the release of a mature egg. It generally occurs around the middle of an individual cycle. Timing varies between women and between cycles.", 
      duration: "Ovulation occurs around the middle of your individual cycle. Its timing can vary.", 
      image: require("../../../../assets/images/explore/menstrual_phase_illustration.jpg") 
    }, 
    food: { 
      keyNutrient: "Balanced Nutrition", 
      description: "There is no requirement for a special 'ovulation diet.' Focus on balanced nutrition.", 
      vegetarian: [
        "Whole grains",
        "Fruits & Veggies",
        "Nuts & Seeds",
  ...
  luteal: { 
    ...
    bg: "bg-[#CBA6F7]/20", 
    overview: { 
      whatHappens: "This phase occurs after ovulation. It continues until the next menstrual bleeding begins. Some women experience physical, emotional or behavioural changes (PMS) during this phase like bloating or fatigue.", 
      duration: "This phase occurs after ovulation and ends when your next menstrual bleeding begins.", 
      image: require("../../../../assets/images/explore/menstrual_phase_illustration.jpg") 
    }, 
    food: { 
      keyNutrient: "Calcium & Vitamin D", 
      description: "You may want to include calcium-rich foods and ensure you get enough Vitamin D.", 
      vegetarian: [
        "Milk & Curd",
        "Paneer",
        "Ragi & Sesame",
...
export default function PhaseDetailScreen() {
  ...
  const data = (phaseContent as any)[phase || "menstrual"] || phaseContent.menstrual;
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "overview":
        return (
          <View className="px-5 pt-6 pb-10">
            <Text className="text-[#1E293B] font-bold text-[18px] mb-3">What happens?</Text>
            ...
            <Text className="text-[#1E293B] font-bold text-[18px] mb-4">How long does it last?</Text>
          ...
          <View className="px-5 pt-6 pb-10">
            <View className="bg-white rounded-[24px] mb-8">
              <View className="flex-row items-center mb-6 pt-2 justify-center">
                <View className={`w-10 h-10 rounded-[12px] ${data.bg} items-center justify-center mr-3`}>
                  <Leaf color={data.color} size={20} />
                </View>
                <View>
                  <Text className="text-[#475569] text-[13px] font-bold mb-1">Key Nutrient to Focus On</Text>
                  <Text className="text-[#1E293B] text-[28px] font-bold">{data.food.keyNutrient}</Text>
                </View>
              </View>
            ...
            <Text className="text-[#1E293B] font-bold text-[18px] mb-4">Food Sources (Indian Examples)</Text>
            ...
            <View className="flex-row rounded-[16px] overflow-hidden mb-8 border border-[#F1F5F9]">
              <View className="flex-1 bg-[#F0FDF4] p-4">
                <Text className="text-[#166534] font-bold text-[15px] mb-4">Vegetarian</Text>
                {data.food.vegetarian.map((item: string, i: number) => (
                  <View key={i} className="flex-row items-start mb-3">
                    <View className="w-1.5 h-1.5 rounded-full bg-[#166534] mt-2 mr-2" />
                    <Text className="text-[#1E293B] text-[14px] leading-[20px] flex-1">{item}</Text>
                  </View>
                ))}
              </View>
              <View className="flex-1 bg-[#FEF2F2] p-4">
                <Text className="text-[#991B1B] font-bold text-[15px] mb-4">Non-Vegetarian</Text>
      ...
      case "activity":
        return (
          <View className="px-5 pt-6 pb-10">
            <Text className="text-[#1E293B] font-bold text-[18px] mb-4">Activity Options</Text>
            <View className="mb-8 pl-2">
              {data.activity.options.map((opt: any, i: number) => (
                <View key={i} className="flex-row items-center py-2">
                  <Text className="text-[22px] w-10 text-center mr-2">{opt.icon}</Text>
                  <Text className="text-[#475569] text-[15px]">{opt.title}</Text>
                </View>
              ))}
            </View>

            <Text className="text-[#1E293B] font-bold text-[18px] mb-4">Yoga Asanas (Options)</Text>
            <View className="flex-row flex-wrap justify-between mb-8">
              {data.activity.yoga.map((asana: any, i: number) => (
                <View key={i} className="w-[48%] bg-[#F8FAFC] rounded-[16px] p-4 items-center mb-3 border border-[#F1F5F9]">
                  <Text className="text-[32px] mb-3">{asana.icon}</Text>
                  <Text className="text-[#475569] text-[13px] text-center font-medium leading-[18px]">{asana.title}</Text>
                </View>
              ))}
            </View>

            <Text className="text-[#1E293B] font-bold text-[18px] mb-4">Breathing Practices (Options)</Text>
            <View className="flex-row flex-wrap justify-between">
              {data.activity.breathing.map((breath: any, i: number) => (
                <View key={i} className="w-[31%] bg-[#F8FAFC] rounded-[16px] p-3 items-center border border-[#F1F5F9]">
                  <Text className="text-[24px] mb-3">{breath.icon}</Text>
                  <Text className="text-[#475569] text-[11px] text-center font-medium leading-[14px]">{breath.title}</Text>
                </View>
              ))}
            </View>
          </View>
      ...
      header={
        <View>
          <View className="pt-1 pb-2">
            <TouchableOpacity onPress={() => router.back()} className="w-12 h-12 p-3 pl-5 z-10 absolute top-1 left-0">
            ...
            <View className="flex-row items-center justify-center mt-2 mb-4">
              ...
              <View>
                 <Text className="text-white font-bold text-[20px] mb-1">{data.title}</Text>
                 <Text className="text-[#8B9CBE] text-[14px]">{data.subtitle}</Text>
              </View>
            </View>
          </View>
